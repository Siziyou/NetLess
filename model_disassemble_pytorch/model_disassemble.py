...
class Predictor():
    def __init__(self,src_model,transformer) -> None:
        self.flatten = False
        self.constr_info = {}
    ...
    def test_load(self, dir):
        ...
        testpng = self.transformer(testpng)
        testpng = testpng.unsqueeze(0)
        self.testpng = testpng
        self.testinput=copy.deepcopy(testpng)
    ...
    def testonnx_func(self, config_dir, model_dir):
        ...
        for i in range(self.constr_info['lenth']):
            self.layer_container.append(torch.load(
                model_dir+"testmodel_"+str(i)+".pth"))
        x2=np.array(self.testpng)
        x = self.testpng
        x3=copy.deepcopy(self.testpng)
        counter=0
        for i in range(self.constr_info['lenth']):
            resnet_session = rt.InferenceSession(model_dir+"testmodel_"+str(i)+".onnx")
...
if __name__ == "__main__":
    ...
    for i in range(1,100):
        print(i)
        new_instance.test_load(PICTURE_DIR + str(i)+".jpg")
        new_instance.testonnx_func(CONFIG_DIR,MODEL_SAVE_DIR)
